...
class ComprehensiveCacheMonitor {
  constructor(client) {
    ...
    this.stats = {
      requests: 0,
      providerHits: 0, // Inferred from response time
      noCache: 0,
      avgResponseTime: 0
    };
  }
  ...
  async makeRequest(messages) {
    ...
    if (responseTime < 50) {
      // Likely cache hit (very fast response)
  ...
  getCacheStats() {
    return {
      ...
      cacheHitRate: `${((this.stats.providerHits / this.stats.requests) * 100).toFixed(1)}%`,
...
- Monitor response times to track cache effectiveness
