...
const completion = await openai.chat.completions.create({
  ...
  models: ["openai:gpt-5-pro", "anthropic:claude-4.5-sonnet"] // Specify allowed models
});
```
...
completion = client.chat.completions.create(
    ...
    messages=[{"role": "user", "content": "Write a story"}],
    models=[
        "openai:gpt-4o-mini",  # All OpenAI models
        "anthropic:claude-3-sonnet"  # Specific model
    ]
)
```
