...
Privacy configuration examples use the OpenAI SDK with Adaptive overrides:
...
const completion = await openai.chat.completions.create({
  model: "adaptive/auto",  // Enable intelligent routing
  messages: [{ role: "user", content: "Sensitive data analysis" }],
  model_router: {
    // Prefer providers with better privacy policies
    models: [
      "z-ai/glm-4.6",              // Zero retention
      "anthropic/claude-haiku-4-5", // Strict privacy guarantees
      "gemini/gemini-2.5-flash-lite" // Regional deployment controls
    ]
  }
});
```
