...
<ParamField body="models" type="array" required>
  ...
  <Expandable title="Model Specification Format">
    **String format (recommended):**
    ```json
    ["openai:gpt-4o-mini", "anthropic:claude-3-5-sonnet-20241022", "google:gemini-1.5-pro"]
    ```

    **Legacy object format (deprecated):**
    ```json
    [{"provider": "openai", "model_name": "gpt-4o-mini"}]
    ```

    The string format is simpler, results in smaller request payloads, and is the recommended approach for all new integrations.
  </Expandable>
</ParamField>
...
response=$(curl -s -w "\n%{http_code}" https://api.llmadaptive.uk/v1/select-model \
...
response=$(curl -s -w "\n%{http_code}" https://api.llmadaptive.uk/v1/select-model \
  ...
  -d '{
    "models": [
      "openai:gpt-4o-mini",
      "local:my-custom-llama-fine-tune"
    ],
    "prompt": "Hello, how are you?"
  }')
...
const res = await fetch('https://api.llmadaptive.uk/v1/select-model', {
  ...
  body: JSON.stringify({
    models: [
      "anthropic:claude-3-5-sonnet-20241022", // Your enterprise contract
      "openai:gpt-4o",    // Your enterprise contract  
      "google:gemini-1.5-pro"     // Pay-per-use fallback
    ],
    prompt: userMessage,
    cost_bias: 0.8
  })
...
const shouldUseCheap = await fetch('https://api.llmadaptive.uk/v1/select-model', {
  method: 'POST',
  headers: { 'X-Stainless-API-Key': adaptiveKey, 'Content-Type': 'application/json' },
  body: JSON.stringify({
...
const shouldUseExpensive = await fetch('https://api.llmadaptive.uk/v1/select-model', {
  method: 'POST',
  headers: { 'X-Stainless-API-Key': adaptiveKey, 'Content-Type': 'application/json' },
  body: JSON.stringify({
    models: ["openai:gpt-4o-mini", "openai:gpt-4o"],
