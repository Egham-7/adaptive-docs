...
<ParamField header="x-goog-api-key" type="string" required>
  Your Adaptive API key. Use `Authorization: Bearer apk_123456` header.
</ParamField>
...
const response = await model.generateContentStream({
  contents: [
    {
      role: 'user',
      parts: [{ text: 'Write a short poem about adaptive AI systems.' }]
    }
  ],
  generationConfig: {
    maxOutputTokens: 512,
...
stream = model.generate_content(
    "Write a short poem about adaptive AI systems.",
    generation_config={
...
```bash cURL (SSE)
curl -N https://api.llmadaptive.uk/v1beta/models/intelligent-routing:streamGenerateContent \
