...
<ParamField body="model_router" type="object">
  ...
  <Expandable title="Properties">
    ...
    <ParamField body="models" type="array">
      Array of allowed provider:model strings for this request.
    </ParamField>
  </Expandable>
</ParamField>
...
response = requests.post(
    ...
    headers={
        'Authorization': 'Bearer apk_123456',  # ← Adaptive key
...
Adaptive returns detailed error information to help you diagnose and resolve issues quickly. While compatible with Anthropic's error format, Adaptive provides additional error types and context.

### Common Errors

See the [complete error reference](/api-reference/errors) for all error types and detailed troubleshooting.
...
**Fallback Failed (503)** - Unique to Adaptive
```json
{
  "type": "error",
  "error": {
...
```json
{
  ...
  "error": {
    ...
    "details": {  // Optional - Adaptive-specific context
...
For Anthropic-specific error handling, see [Error Handling Best Practices](/guides/error-handling).
