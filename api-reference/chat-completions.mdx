...
<ParamField body="model" type="string" required>
  ...
  **For specific models**: Use provider:model format like `"anthropic:claude-sonnet-4-5"` or `"openai:gpt-5-mini"`
</ParamField>
...
### Adaptive-Specific Errors

<Warning>
**🔄 Unique to Adaptive**: These errors provide insights not available from other AI APIs. They help you understand multi-provider failures and make intelligent retry decisions.
</Warning>

<AccordionGroup>
<Accordion title="🔄 Fallback Error (503) - All Providers Failed">
**Adaptive's Signature Feature**: When intelligent routing exhausts all provider options.
...
**What This Tells You:**
- **Provider Details**: Which providers were tried and why they failed
- **Performance Insights**: How long each provider took (`duration_ms`)
- **Failure Patterns**: All rate limits? Mixed errors? Service outages?

**Recovery Strategies:**
1. **All Rate Limits**: Implement exponential backoff (1s → 2s → 4s)
2. **Mixed Errors**: Check [status page](https://status.llmadaptive.uk) for outages
3. **High Duration**: Consider race mode for faster failure detection
...
```javascript
  ...
  if (error.response?.data?.error?.type === 'fallback_failed') {
    ...
    const allRateLimited = failures.every(f => f.error.includes('rate limit'));
    ...
    if (hasOutage) {
      // Use cached response or simplified answer
...
**See:** [Complete Error Reference](/api-reference/errors#fallback-errors-503-unique-to-adaptive)
</Accordion>

<Accordion title="🔍 Model Registry Error (404)">
**Problem**: Model lookup failed in Adaptive's registry.
...
2. **Use intelligent routing**: Set `model: ""` instead of specific model
3. **Verify model name** from [supported models](/supported-models) page
</Accordion>

<Accordion title="🧭 Model Router Error (503)">
**Problem**: Intelligent routing couldn't select an appropriate model.
...
3. **Retry with backoff** - this is often transient
</Accordion>
</AccordionGroup>
