  ...
  **Control intelligent routing** to optimize cost and performance
  ...
  <Expandable title="Full Configuration">
    ...
    <ParamField body="models" type="array">
      ...
      - `"anthropic:claude-3-5-sonnet-20241022"` - Specific model
      - `"google:gemini-1.5-pro"` - Specific model
    </ParamField>
...
<ParamField body="model_router" type="object">
  ...
  <Expandable title="Model Router Config">
    <ParamField body="models" type="ModelCapability[]">
      ...
      **Simple formats:**
      - `"openai:gpt-4o-mini"` - Use specific OpenAI model
      - `"anthropic:claude-3-sonnet-20240229"` - Use specific Anthropic model
      ...
      **Custom models require all parameters:**
      ...
      <Expandable title="Model Capability Object">
        ...
        <ParamField body="model_name" type="string">
          Specific model identifier (e.g., `"gpt-4"`, `"claude-3-sonnet"`). **Required for specific model selection**
        </ParamField>
...
const completion = await openai.chat.completions.create({
  ...
  model_router: {
    models: [
      "anthropic:claude-3-5-sonnet-20241022", // All Anthropic models
      "openai:gpt-4" // Specific OpenAI model
    ],
...
const completion = await openai.chat.completions.create({
  ...
  model_router: {
      models: [
        "openai:gpt-4o-mini", // Will use custom config below
        "anthropic:claude-3-5-sonnet-20241022" // Will also use custom config
      ]
  },
...
const completion = await openai.chat.completions.create({
  ...
  model_router: {
    models: [
      "openai:gpt-4o-mini", // Use all OpenAI models
      "anthropic:claude-3-sonnet-20240229", // Specific model
    ],
    cost_bias: 0.3,
