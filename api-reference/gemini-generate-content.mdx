...
<Note>
Prefer environment variables? Set `GOOGLE_GEMINI_BASE_URL=https://api.llmadaptive.uk/v1beta` so any Gemini-compatible SDK automatically points to Adaptive's endpoint.
</Note>
...
```json
{
  "error": {
    "code": 401,
    "message": "API key required. Provide it via Authorization: Bearer header",
    "status": "UNAUTHENTICATED"
  }
}
```
**Solution**: Provide a valid API key in the `Authorization: Bearer` header.
</Accordion>
