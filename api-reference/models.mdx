...
4. **Custom Models**: You can specify custom models not in the registry with your own details

### Registry vs Manual Specification
...
**Manual Specification (Custom Models):**
```json
{
  "models": [
    {
...
<RequestExample>
```bash cURL
curl https://api.llmadaptive.uk/v1/models/gpt-4o-mini \
  -H "Authorization: Bearer $ADAPTIVE_API_KEY"
```
...
response = requests.get(
    "https://api.llmadaptive.uk/v1/models/gpt-4o-mini",
    headers=headers
)
model = response.json()
